...
// --- COMPONENT: Map Revalidator (Fixes Gray Tiles & Loading Issues) ---
...
// Color mapping function for activities (Moved outside component for stability)
...
// Custom function to create div icons for the map (Moved outside)
...
// Keeps icon reference stable to prevent Leaflet "reading '_leaflet_pos'" error on re-renders
const TripMarker = React.memo(({ trip, t, onDetail, onBook, onGuideClick }: { trip: Trip, t: any, onDetail: () => void, onBook: () => void, onGuideClick: (e: any) => void }) => {
  const icon = useMemo(() => createCustomIcon(trip.price, trip.activityType), [trip.price, trip.activityType]);

  return (
    <Marker 
        position={[trip.coordinates.lat, trip.coordinates.lng] as L.LatLngExpression}
        icon={icon}
    >
        <Popup className="custom-popup" closeButton={false} maxWidth={300} minWidth={280}>
           {/* Add pointer-events-auto to ensure clicks register inside popup */}
           <div className="p-0 pointer-events-auto">
                ...
                <div className="p-3">
                    ...
                    <div className="flex gap-2">
                        <button 
                            // stopPropagation for both click and touch events to avoid map interaction
                            onClick={(e) => { e.stopPropagation(); onDetail(); }}
                            onMouseDown={(e) => e.stopPropagation()}
                            onTouchStart={(e) => e.stopPropagation()}
                            className="flex-1 bg-white border border-slate-200 text-slate-700 text-xs font-bold py-2 rounded-lg hover:bg-slate-50 transition-colors flex items-center justify-center gap-1 touch-manipulation"
                        >
                        ...
                        <button 
                            ...
                            onMouseDown={(e) => e.stopPropagation()}
                            onTouchStart={(e) => e.stopPropagation()}
                            className="flex-1 bg-slate-900 text-white text-xs font-bold py-2 rounded-lg hover:bg-slate-800 transition-colors touch-manipulation"
                        >
                            {t.book}
                        </button>
                    </div>
...
const Marketplace: React.FC<Props> = ({ trips, onRequestJoin, lang }) => {
  ...
  const handleGuideClick = (e: React.MouseEvent, trip: Trip) => {
      e.stopPropagation();
      e.preventDefault(); // Prevents map clicks
            ...
            >
                ...
                {filteredTrips.map(trip => (
                    <TripMarker 
                        ...
                        trip={trip}
                        t={t}
                        onDetail={() => setViewDetailsTrip(trip)}
                        onBook={() => setBookingModalTrip(trip)}
                        onGuideClick={(e: any) => handleGuideClick(e, trip)}
                    />
                ))}
            </MapContainer>
